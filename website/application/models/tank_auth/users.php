...
class Users extends CI_Model
{
	...
	function count_login($user_id) {
		$previous = $this->db->select('login_number')->where('id', $user_id)->get($this->table_name)->row();
	
		if (is_null($previous->login_number)) {
			$count = 0 + 1;
		} else {
			$count = $previous->login_number + 1;
		}
	
		$this->db->set('login_number', $count);
		return $this->db->where('id', $user_id)->update('users');
	}
	
	function get_login_counts() {
		$this->db->select('email, login_number');

...
class Tank_auth
{
			...
			if (!is_null($user = $this->ci->users->$get_user_func($login))) {	// login ok
				...
				if ($hasher->CheckPassword($password, $user->password)) {		// password ok
					...
					if ($user->banned == 1) {									// fail - banned
						...
						$profile = $this->ci->user_profiles_model->get_by_user_id($user->id);
	...
	public function account_is_prepared()
	{
		...
		$user_profile = $this->ci->user_profiles_model->get_by_user_id($this->get_user_id());

		// If user is not connected to at least Twitter
		$accounts = $this->ci->user_accounts_model->get_by_user_id($this->get_user_id());
		$hasTwitter = false;

		foreach ($accounts as $account) {
			if ($account->type == 'TWITTER' && (!is_null($account->access_token) && $account->access_token != 'N;'))
			{
				$hasTwitter = true;
				break;
			}
		...
		// If user haven't filled out location
		if (is_null($user_profile->user_lat) || is_null($user_profile->user_lon) || is_null($user_profile->user_location))
		{
			return false;
		}

		// If we came so far, the user is prepared
		return true;
	}
}
